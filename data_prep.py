...
# clean csv to showcase only trackable stock trades i.e in stock market 
# database (ONLY PURCHASE AND SALE TYPE)
...
s_trans = s_trans.drop(columns=['comment', 'ptr_link']).drop_duplicates()
s_trans.reset_index(drop=True, inplace=True)
...
# set interval column dates to be 3 months before and after purchase date, 
# except when three months past is a future date (then set as current date)
s_trans['period_end'] = np.where(s_trans['transaction_date'] + three_month >=
                                 datetime.date.today(),
                                 datetime.date.today(), 
